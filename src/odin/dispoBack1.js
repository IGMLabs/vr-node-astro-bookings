...
if (cluster.isMaster) {
  ...
  var arrayClaves = []; // guardaremos en un array las claves de las petis ( q sero?= un no?=mero), para poder acceder
  ...
  requestCodigos.receiveMessage(args => {
    ...
    if (tipo == 'Inicio') {
      for (let i = 0; i < global.NODE_URL_BROKERS.length; i++) {
        ...
        console.log(idWorker, ' ... inicializacion completa! Router en ' + global.NODE_URL_BROKERS[i] + global.PROCESAR_PORT);
      }
  ...
  for (let i = 0; i < socket.length; i++) {
    (socket_id => {
      ...
      socket[socket_id].receiveMessage(function (args) {
        ...
        if (msg.scope.timetoOut) {
          msg.scope.timetoOut = Math.round((msg.scope.timetoOut / 1000) * 100) / 100;// para pasar a segundos y redondear a dos decimales
        } else {
          msg.scope.timetoOut = global.REQUEST_TTL;
        }
  ...
  function tratarDispo(scope, callback) {
    ...
    cache.set("" + posKeys, {
      ...
      timeOut: () => {
        ...
        console.log(scope.dur + '\t' + petisProcesando + '\t-1\t-1\t-1\t-1\t' + (Date.now() - scope.dur) + '\t' + id + '\t' + numPeticiones + '\t' + cache.getStats().keys + '\t' + Object.keys(scope.entNmPaxes).length + '\t' + 'TIMEOUT');
      },
      exec: () => {
        petisProcesando++;
        //scope.entHoteles = scope.entHoteles.map(Number);
        const databaseTime = Date.now();
        database.findPlanning(new Date(scope.entFecEntr), scope.entHoteles, scope.entNumNocs, (error, results, databaseOrigin) => {
          ...
          if (error) {
            ...
            console.log(scope.dur + '\t' + petisProcesando + "\t" + (scope.entHoteles.length * scope.entNumNocs) + "\t" + 0 + "\t" + Date.now() - processTime + "\t" + processTime - databaseTime + '\t' + (Date.now() - scope.dur) + '\t' + id + '\t' + numPeticiones + '\t' + cache.getStats().keys + '\t' + Object.keys(scope.entNmPaxes).length + '\t' + 'DB_ERROR' + '\t' + databaseOrigin);
            eventLista.emit('Next');
          } else {
            procesarDispo(results, scope, databaseOrigin, availability => {
              callback(null, availability, "OK");
              let resultsWithAvailability=0;
              if(databaseOrigin == "mongo"){
                resultsWithAvailability = results.length;
              ...
              console.log(scope.dur + '\t' + petisProcesando + "\t" + (scope.entHoteles.length * scope.entNumNocs) + "\t" + resultsWithAvailability + "\t" + (Date.now() - processTime) + "\t" + (processTime - databaseTime) + '\t' + (Date.now() - scope.dur) + '\t' + id + '\t' + numPeticiones + '\t' + cache.getStats().keys + '\t' + Object.keys(scope.entNmPaxes).length + '\t' + "OK" + '\t' + databaseOrigin);
              eventLista.emit('Next');
            });
          }
        });
      }
    }, scope.timetoOut, (err, success) => {
      if (success == false) {
        console.error("ERROR CACHE SET: " + err + "--" + success);
      }
    });
    posKeys++;
    reinicio++;
    // por si en algun remoto caso nos estancamos y no seguimos procesando petis , esto haro?= que poco a poco se destrabe
    if ((reinicio % 10 < 2) || (!global.MAX_PETIS_PROCESS) || (arrayClaves.length > 0 && petisProcesando < global.MAX_PETIS_PROCESS)) {
  ...
  function procesarDispo (infoDispo, scope, databaseOrigin, callback){
    ...
    infoDispo.forEach(function (dispoTratamiento) {
      if (databaseOrigin == "mongo") {
        dispoTratamiento = bson.deserialize(dispoTratamiento);
      } else if (databaseOrigin == "redis") {
        if (!dispoTratamiento) {
          return;
        }
      ...
      if (Date.now() - scope.entTimeIni > scope.entTimemax - 20 - 80) {
        setHotelesTO.add(dispoTratamiento.hot);
      } else {
  ...
  function procesarDispoHotel (dispoTratamiento, scope, resDispoHoteles){
    ...
    if(!scope.entNmPaxes){ // Parche temporal
      scope.entNmPaxes ={"1-0":1};
    }

    //TODO eliminar packs menores
    Object.keys(scope.entNmPaxes).sort().forEach(function(codPaxPeticion){
      ...
      for (var i = 0; i < arrayPax.length; i++) {
        ...
        if(utils.isPaxAccepted(paxAdultos, paxNinos, paxPetAdultos, paxPetNinos)){
          ...
          var resBusqueda = procesarDispoHotelPorPax(dispoTratamiento, scope, numHabitaciones, arrayHabitaciones, infoHabitacion);

          if (codPaxPeticion == pax) resDelPaxOriginal = resBusqueda;
          var hayHabitacionDispo = false;
          arrayHabitaciones.forEach(function (codHab) { if (resBusqueda[codHab].hayDispo) hayHabitacionDispo = true });
      ...
      if (resDelPaxOriginal) insertarDispo(resDelPaxOriginal, codPaxPeticion, codPaxPeticion, diferentesPax[codPaxPeticion]);
    });
    ...
    function insertarDispo(resBusqueda, codPaxPeticion, codPaxOrigen, arrayHabitaciones){
      ...
      if (!objHotel) { objHotel = {}; resDispoHoteles[dispoTratamiento.hot] = objHotel }
      var objPax = objHotel[codPaxPeticion];
      if (!objPax) { objPax = {}; objHotel[codPaxPeticion] = objPax }
      ...
      Object.keys(inventarioCogido).forEach(function(origen){
        if (dispoTratamiento.inv[origen].est != 'V') {
          dispoTratamiento.inv[origen].nHab -= inventarioCogido[origen];
        }
      });
    }
  }

  function procesarDispoHotelPorPax(dispoTratamiento, scope, numHabitaciones, arrayHabitacionesPeticion, infoHabitacion){
    var resBusqueda = {};
    ...
    dispoTratamiento.dtos && dispoTratamiento.dtos.forEach(function (dscto) {
      let push = true;
      if (dscto.rest) { // restricciones
        // Dias / Habitaciones
        push = descuentos.calculaDescuento(dscto,numHabitaciones,scope.entNumNocs,diasAntelacion,dispoTratamiento.afi,afilAge,scope.entHoraPro,scope.entFechPro,gruposAgencia,scope.entCobolAg);
      }
    ...
    arrayHabitacionesPeticion.forEach(function (codHabitacion) {
      ...
      Object.keys(objHabitacion).forEach(function (codTarifa) {
        ...
        if(!infoTarifa.divN){
          ...
          infoTarifa = {divN:tar[0],iIva:tar[1]=='S',pIva:tar[2],pPor:tar[3],comi:tar[4],comS:tar[5],aumA:tar[6],fit:tar[7]=='S', regMin:pesoReg[tar[10]]};
        ...
        if(aumentado_general == 0) {
          aumentado_general = scope.entAumFich;
        }

        var ratioAumAgencia = agencia.getRatioAumAgencia(codigos.tarifas[dispoTratamiento.afi+ codTarifa.trim()],infoTarifa,scope.entAumefit,scope.entAumOfit,scope.entAumInt5,scope.entAgraFit,dispoTratamiento.hot,dispoTratamiento.pais,scope.PVPVinculado,scope.entNetas);
        var porcComAgencia = agencia.getComisionAgencia(codigos.comEspeciales[dispoTratamiento.hot] , codigos.comEspeciales[dispoTratamiento.hot+'#'+dispoTratamiento.afi] , codigos.comEspeciales[dispoTratamiento.hot+'#'+scope.entCobolAg+'#'+scope.entAfilAge] , dispoTratamiento.pais,scope.entComAgen[afilAge],infoTarifa.comi,infoTarifa.fit,aumentado_general,scope.PVPVinculado,scope.entNetas);
        ...
        var gastosCalc = gastos.calcGastosCalc(codigos.tarifas[dispoTratamiento.afi+codTarifa],codigos.gastosCancelacionMarca,dispoTratamiento.dia,dispoTratamiento.afi,dispoTratamiento.pais,dispoTratamiento.gtosT,dispoTratamiento.gtos,codTarifa,scope.entNumNocs);
        //TODO:calcular precios RO OB BB

        Object.keys(objTarifa).forEach(function (codRegimen){
          var hayRestric = true;
          if(codRegimen=='ori') return;        var estado = dispoTratamiento.inv[objTarifa.ori].est; // P:peticion || C:cupo || V:venta libre TODO:Eliminar variable
          var precioAumNorm = calcPrecioUnitario(dispoTratamiento.afi, objTarifa[codRegimen],infoHabitacion, infoTarifa);
          var precioAumentado = calcPrecioDivisa(dispoTratamiento.afi, objTarifa[codRegimen],infoHabitacion, infoTarifa, divisaDestino);
          ...
          if(scope.entAgraFit[dispoTratamiento.hot].pvpiva == undefined){
            pvpiva = true;
          } else {
            pvpiva = scope.entAgraFit[dispoTratamiento.hot].pvpiva
          }


          if(infoTarifa.regMin > pesoReg[codRegimen]) return;

          if (isrest) hayRestric = restricciones.restricciones(isrest,codHabitacion,codTarifa,codRegimen,scope.entNumNocs,dispoTratamiento.dia,scope.entFecEntr,scope.FecSald);
          ...
          if (scope.PVPVinculado){
            PVPVinculadoTarifa = precioAumentado.precio + precioAumentado.iva;
          }
          ...
          if (infoTarifa.comS=='T' || (infoTarifa.comS!='T' && (codRegimen=='OB' || codRegimen=='RO' || (!objTarifa['RO'] && !objTarifa['OB']) ))){ // Sobre el total
            impComision = agencia.comisionAgencia(afilAge,impComision,precioAumentado,porcComAgencia);
            if (scope.entNetas){
                precioAumentado.precio -= impComision;
            }

          }else{ // Sobre el alojamiento
            var imporSoloAlojamiento = objTarifa['RO'] || objTarifa['OB'] || 0;
            imporSoloAlojamiento = calcPrecioDivisa(dispoTratamiento.afi,imporSoloAlojamiento,infoHabitacion, infoTarifa, divisaDestino);
          ...
          if (infoTarifa.iIva && !pvpiva) precioAumentado.precio = precioAumentado.precio / ( 1 + (infoTarifa.pIva/100))
          ...
          dsctosGenerales.forEach(function (dscto){
            ...
            if (dscto.porc || dscto.impo) {
              ...
              var impDescuento = 0;
              if (dscto.porc) {
                //En el caso de que el descuento sea <1 no se daro?= la tarifa como reembolsable.
                ...
                if (dscto.base == 'A' || dscto.base == 'D') {
                  ...
                  if (origenAlojamiento) {
                    var precioAlojamiento = calcPrecioDivisa  (dispoTratamiento.afi,origenAlojamiento,infoHabitacion, infoTarifa, divisaDestino);
                    ...
                    if (dscto.base == 'A') {
                      ...
                      impDescuento = (scope.entNetas)?precioAlojamiento * (1-porcComAgencia/100) *  (dscto.porc / 100) / ratioAumAgencia.agencia
                          :precioAlojamiento * (dscto.porc / 100) / ratioAumAgencia.agencia;
                      PVPVinculado -= precioAlojamiento *  (dscto.porc / 100);
                    }else{
                      if(codRegimen!='OB' && codRegimen!='RO'){
                        var origenAlojamientoDesayuno = objTarifa['BB'];
                        if (origenAlojamientoDesayuno) {
                          var precioDesayuno = calcPrecioDivisa(dispoTratamiento.afi, origenAlojamientoDesayuno, infoHabitacion, infoTarifa, divisaDestino);
                          precioDesayuno = precioDesayuno.precio + precioDesayuno.iva;
                          impDescuentoNorm =
                              (calcPrecioUnitario(dispoTratamiento.afi,origenAlojamientoDesayuno,infoHabitacion, infoTarifa) -
                              calcPrecioUnitario(dispoTratamiento.afi,origenAlojamiento, infoHabitacion, infoTarifa))
                              * (1-porcComAgencia/100) * (dscto.porc / 100) / ratioAumAgencia.agencia;

                          impDescuento = (scope.entNetas)?(precioDesayuno - precioAlojamiento) * (1-porcComAgencia/100) * (dscto.porc / 100) / ratioAumAgencia.agencia
              ...
              if (precioAumNorm > 2 && precioAumentado.precio > 2 && impDescuentoNorm > 0 && impDescuento > 0 && precioAumNorm - impDescuentoNorm > 2 ) {

                var precio = (precioAumentado.precio - impDescuento) * (numHab || numHabitaciones);


                if(dsctEmpBase && !infoTarifa.fit){
                  precio *= (1-(dsctEmpBase/100));
                }
                ...
                emitirTarifa(
                    ...
                    diasAntelacion,
                    {
                      ...
                      tarifa : {
                        ...
                        gastos: gastos.compGasTarifaMarcaFinal(scope.entNumNocs, [(dscto.gtos && JSON.parse('[' + dscto.gtos + ']')), gastosCalc])
                      }
                    }
                )
              }
          ...
          if (precioAumNorm > 2 && precioAumentado.precio > 2 && (scope.entPreAlte && resBusqueda[codHabitacion].precioConAlternativa)) {
  ...
  function emitirTarifa(entReembol, resBusqueda, numHabitaciones, diasAntelacion, objInfo){
      ...
      if (!objHabit) { objHabit = {}; resBusqueda.dispo[objInfo.habit] = objHabit }
      var objRegim = objHabit[objInfo.regim];
      if (!objRegim) { objRegim = {}; objHabit[objInfo.regim] = objRegim }

      if (entReembol == 0 || entReembol == 3) {
  ...
  function incluirTarifa(resBusqueda, numHabitaciones, objRegim, nr, objInfo){
    ...
    if (!objCond) { objCond = {dias:{}} ; objRegim['nr'+ nr + '#rest0'] = objCond }
    objDia = objCond.dias[objInfo.dia];
    if (!objDia) { objDia = {faltan:numHabitaciones, tarifas:[]} ; objCond.dias[objInfo.dia] = objDia }
  ...
  function calcularDia(resBusqueda, numHabs, objInfo, objDia){
    ...
    var insertarTarifa = function (numHabsFaltan, tarifa) {
      ...
      if (!tarifa.numHab || numHabs-numHabsACoger>0) {  // Solo se inserta la primera tarifa a peticion existente mas barata
    ...
    for (var i = 0; i< objDia.tarifas.length;i++) {
      ...
      if (!tarInsertada){
        if(objInfo.tarifa.precNorm < tarExistente.precNorm){
          tarInsertada = true;
        }else if(objInfo.tarifa.precNorm == tarExistente.precNorm){
          if(objInfo.tarifa.dsctBase && (objInfo.tarifa.dsctBase > tarExistente.dsctBase)){
            tarInsertada = true;
          }else if(objInfo.tarifa.dsctBase && (objInfo.tarifa.dsctBase == tarExistente.dsctBase)){
            if (objInfo.tarifa.dsctPorc && (objInfo.tarifa.dsctPorc > tarExistente.dsctPorc)){
              tarInsertada = true;
            }else if(objInfo.tarifa.dsctPorc && objInfo.tarifa.dsctPorc == tarExistente.dsctPorc){
              if(objInfo.tarifa.gastos[0] < tarExistente.gastos[0]){
                tarInsertada = true;
              }
            }
          }
      ...
      if (objInfo.tarifa.origen != tarExistente.origen) {
  ...
  function calcPrecioDivisa(afil, regim, infoHabitacion, infoTarifa, divisaDestino){
    ...
    if (infoTarifa.iIva){
        prec.precio -= prec.iva;
    }  else {
        prec.iva = prec.precio * (infoTarifa.pIva/100) ;
    }
    ...
    if (divisaOrigen != divisaDestino) {
      if (divisaOrigen != 'EU') {
        if (!codigos.divisas.hasOwnProperty(afiliacion + '#' + divisaOrigen)) {
          if (process.env.NODE_ENTORNO == 'PROD' || true) console.log ('ATENCION!! No existe la divisa ', divisaOrigen, ' para la afiliacion ', afiliacion);
          return 0;
        }
      ...
      if (divisaDestino != 'EU') {
        if (!codigos.divisas.hasOwnProperty(afiliacion + '#' + divisaDestino)) {
          if (process.env.NODE_ENTORNO == 'PROD' || true) console.log ('ATENCION!! No existe la divisa ', divisaDestino, ' para la afiliacion ', afiliacion);
          return 0;
        }
  ...
  function calcPVPVinculado(hotel,fechaSist) {
    if (codigos.hotelesPVP['PVP'].indexOf(hotel) != -1) {
      return true;
    }
    ...
    if (codigos.hotelesPVP[hotel]) {
      ...
      if (objPVPVinc) {
        if (fechaSist > utils.HHMMtoDate(objPVPVinc.hIni, fechaSist) && fechaSist < utils.HHMMtoDate(objPVPVinc.hFin, fechaSist)) {
          return true;
        }
      }
    }
          ...
          Object.keys(hab).forEach(function (codReg){
            ...
            Object.keys(reg).forEach(function (codCond){
              ...
              Object.keys(objCond.dias).forEach(function (dia) {
                ...
                if (!objCond.gastos || !objCond.gastos.length || (dia.tarifas[0].gastos && objCond.gastos[0] < dia.tarifas[0].gastos[0])){
                  objCond.gastos = dia.tarifas[0].gastos;
                }
              });

              if (scope.entCadaDia && Object.keys(objCond.dias).length < scope.entNumNocs){
                delete reg[codCond];
              }else if(!scope.entIncPeti && !objCond.tieneDispo){
                delete reg[codCond];
              } else {
                if (arrCond[0] == 'nr-1') {
                  arrCond[0] =  'nr0';
                  if (objCond.gastos)
                    if (scope.entReembol == 0) {
                      ...
                      switch (gastos.entraEnGastos(objCond.gastos, scope.entDiasAnt,true)){
                        case 1:  arrCond[0] = 'nr1'; break;
                        case -1: arrCond[0] = 'nr2'; break;
                      }
                    }
                  reg[arrCond.join('#')] = reg[codCond];
                  delete reg[codCond];
                }else{
                  if (arrCond[0] == 'nr0' && objCond.gastos && gastos.entraEnGastos(objCond.gastos, scope.entDiasAnt)) delete reg[codCond]; // NUNCA DEBERIA ENTRAR POR AQUI, SERIA UN ERROR.
                  if (arrCond[0] == 'nr1' && (!objCond.gastos || !gastos.entraEnGastos(objCond.gastos, scope.entDiasAnt))) delete reg[codCond];
                  if (arrCond[0] == 'nr2' && (!objCond.gastos || gastos.entraEnGastos(objCond.gastos, scope.entDiasAnt,true)!=-1)) delete reg[codCond];
