...
if (cluster.isMaster) {
  ...
  function tratarDispo(scope, callback) {
    ...
    if (
      reinicio % 10 < 2 ||
      !global.MAX_PETIS_PROCESS ||
      (arrayClaves.length > 0 && petisProcesando < global.MAX_PETIS_PROCESS)
    )
...
// PARTE 1 A MODIFICAR PROCESARDISPO
  ...
  /**creamos la funcion dispoTratamiento */
  function dispoTratamiento(infoDispo, databaseOrigin) {
    infoDispo.forEach(function (dispoTratamiento) {
      dispoTratamiento = compruebaDataBaseOrigin(databaseOrigin);
  ...
  /**COMPROBAMOS LA BASE DE DATOS ORIGIN */
  function compruebaDataBaseOrigin(databaseOrigin) {
    if (databaseOrigin == "mongo") {
      return bson.deserialize(dispoTratamiento);
    }
    if (databaseOrigin == "redis") {
      if (!dispoTratamiento) {
        return;
      }
      return JSON.parse(dispoTratamiento);
    }
  }

  //PARTE 2 A MODIFICAR PROCESARDISPOHOTEL
  function procesarDispoHotel(dispoTratamiento, scope, resDispoHoteles) {
    //Packs disponibles en MONGO segun habitacion
    var diferentesPax = compruebaPaxes(Object.keys(dispoTratamiento.habs));
  ...
  function procesarDispoHotelPorPax(
    ...
    const dsctEmpBase =
      scope.entAgraFit[dispoTratamiento.hot] &&
      scope.entAgraFit[dispoTratamiento.hot].dtoEmp &&
      scope.entAgraFit[dispoTratamiento.hot].dtoEmp[offset];
    var dsctosGenerales = [];
    ...
    dispoTratamiento.dtos &&
      dispoTratamiento.dtos.forEach(function (dscto) {
        let push = true;
        if (dscto.rest)
          // restricciones
          // Dias / Habitaciones
    ...
    arrayHabitacionesPeticion.forEach(function (codHabitacion) {
      ...
      Object.keys(objHabitacion).forEach(function (codTarifa) {
        ...
        var divisaDestino =
          infoTarifa.divN != "EU" && scope.entDivPerm && scope.entDivPerm.indexOf(infoTarifa.divN) > -1
            ? infoTarifa.divN
            : "EU";
        ...
        Object.keys(objTarifa).forEach(function (codRegimen) {
          ...
          if (
            infoTarifa.comS == "T" ||
            (infoTarifa.comS != "T" &&
              (codRegimen == "OB" || codRegimen == "RO" || (!objTarifa["RO"] && !objTarifa["OB"])))
          ) {
            // Sobre el total
          ...
          dsctosGenerales.forEach(function (dscto) {
            ...
            if (dscto.porc || dscto.impo) {
                  ...
                  if (origenAlojamiento) {
                    ...
                    if (dscto.base == "A") {
                      ...
                      impDescuento = scope.entNetas
                        ? (precioAlojamiento * (1 - porcComAgencia / 100) * (dscto.porc / 100)) /
                          ratioAumAgencia.agencia
              ...
              if (
                precioAumNorm > 2 &&
                precioAumentado.precio > 2 &&
                impDescuentoNorm > 0 &&
                impDescuento > 0 &&
                precioAumNorm - impDescuentoNorm > 2
              ) {
          ...
          if (
            ...
            precioAumentado.precio > 2 &&
            scope.entPreAlte &&
            resBusqueda[codHabitacion].precioConAlternativa
          ) {
          ...
          Object.keys(hab).forEach(function (codReg) {
            ...
            Object.keys(reg).forEach(function (codCond) {
              ...
              Object.keys(objCond.dias).forEach(function (dia) {
                ...
                if (
                  !objCond.gastos ||
                  !objCond.gastos.length ||
                  (dia.tarifas[0].gastos && objCond.gastos[0] < dia.tarifas[0].gastos[0])
                )
